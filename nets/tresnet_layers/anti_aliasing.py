...
class DownsampleJIT(object):
    ...
    def __call__(self, input: torch.Tensor):
        ...
        return F.conv2d(input_pad, self.filt.to(input_pad.device), stride=2, padding=0, groups=input.shape[1])
...
class Downsample(nn.Module):
    ...
    def forward(self, input):
        input_pad = F.pad(input, (1, 1, 1, 1), 'reflect')
        return F.conv2d(input_pad, self.filt.to(input_pad.device), stride=self.stride, padding=0, groups=input.shape[1])
