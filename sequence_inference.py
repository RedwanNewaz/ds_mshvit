...
def evaluate(dataloader, rgb_backbone, rgb_head, seq_backbone, seq_head, act_func, device, alpha):
    rgb_backbone.eval()
    rgb_head.eval()
    seq_backbone.eval()
    seq_head.eval()
    ...
    with torch.no_grad():
        for i, (X, Y, paths ) in enumerate(dataloader):
            ...
            feat_map2 = seq_backbone(x2)
            logits2 = seq_head(feat_map2)
...
def inference(args):
    ...
    if bargs.backbone_model in backbones.VALID_BACKBONES:
        ...
        seq_backbone = backbones.get_backbone(backbone_model=bargs.backbone_model,
                                              img_size=bargs.img_size,
                                              return_stages=bargs.backbone_feature_maps,
                                              pretrained_backbone=bargs.pretrained_backbone_seq,
                                              custom_pretrained_path="")
    ...
    if hargs.head_model in heads.HEADS:
        if hargs.head_model == "BaseHead":
            ...
            seq_head = heads.BaseHead(num_classes=hargs.num_classes,
                                      backbone_feature_map_sizes=seq_backbone.feature_map_sizes,
                                      last_stage=seq_backbone.last_stage,
                                      global_pool=hargs.global_pool,
                                      sigmoid_loss=hargs.sigmoid_loss,
                                      tresnet_init=hargs.tresnet_init)
            ...
            seq_head = heads.MultiScaleViTHead(num_classes=hargs.num_classes,
                                               token_dim=hargs.token_dim,
                                               representation_size=hargs.representation_size,
                                               tokenizer_layer_name=hargs.tokenizer_layer,
                                               block_type=blocks.__dict__[hargs.block_type],
    ...
    if use_ema:
        ...
        seq_updated_backbone_state_dict = best_model["seq_state_dict_backbone_ema"]
        seq_updated_head_state_dict = best_model["seq_state_dict_head_ema"]
    else:
        print("NO EMA")
        ...
        seq_updated_backbone_state_dict = best_model["seq_state_dict_backbone"]
        seq_updated_head_state_dict = best_model["seq_state_dict_head"]
    
    rgb_backbone.load_state_dict(rgb_updated_backbone_state_dict)
    seq_backbone.load_state_dict(seq_updated_backbone_state_dict)

    rgb_head.load_state_dict(rgb_updated_head_state_dict)
    seq_head.load_state_dict(seq_updated_head_state_dict)
    ...
    seq_backbone = seq_backbone.to(device)
    seq_head = seq_head.to(device)
    ...
    if eval_val:
        ...
        val_predictions, val_imgPaths = evaluate(val_dataloader, rgb_backbone, rgb_head, seq_backbone, seq_head, act_func, device, alpha)
    ...
    if eval_test:
        ...
        test_predictions, test_imgPaths = evaluate(test_dataloader, rgb_backbone, rgb_head, seq_backbone, seq_head, act_func, device, alpha)
