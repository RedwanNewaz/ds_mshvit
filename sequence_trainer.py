...
class Model(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.valid_f2ciw(y_hat, y1)
        self.valid_f1normal(y_hat, y1)
        ...
        if self.hparams.args.model_ema and not self.hparams.args.model_ema_force_cpu:
            ...
            with torch.cuda.amp.autocast(enabled=self.autocast_loss):
                total_loss_ema = self.criterion_labels(logits1_ema, y1) + \
                                 self.criterion_labels(logits2_ema, y1) + \
            ...
            self.valid_f2ciw_ema(y_hat_ema, y1)
            self.valid_f1normal_ema(y_hat_ema, y1)
