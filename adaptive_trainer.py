...
class Model(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        with torch.cuda.amp.autocast(enabled=self.autocast_loss):
            ...
            attention_loss = self.criterion_feats(feat_map1[self.attention_layer], feat_map2[self.attention_layer])
            total_loss = rgb_loss +  opti_loss + attention_loss
            
            rgb_loss_val = rgb_loss.detach().cpu().numpy()
            opti_loss_val = opti_loss.detach().cpu().numpy()

            if rgb_loss_val < opti_loss_val :
                self.alpha = min(self.alpha + self.alphaLearningRate, 0.99)
            elif rgb_loss_val > opti_loss_val:
    ...
    def validation_step(self, batch, batch_idx):
        ...
        with torch.cuda.amp.autocast(enabled=self.autocast_loss):
            ...
            total_loss = self.criterion_labels(logits1, y) + \
                         self.criterion_labels(logits2, y) + \
                         self.criterion_feats(feat_map1[self.attention_layer], feat_map2[self.attention_layer])
...
def main(args):
    ...
    dm = datamodules.get_joint_datamodule(dataset=args.dataset, batch_size=args.batch_size, workers=args.workers,
                                          ann_root=args.ann_root, rgb_data_root=args.rgb_data_root,
                                          opti_data_root=args.opti_data_root, train_transform=train_transform,
