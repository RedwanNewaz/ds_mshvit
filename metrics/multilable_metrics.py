...
def average_precision(scores, target, max_k=None):
    ...
    # print(scores.shape, total_count, total_cases, pos_count, max_k)
...
def per_class_metrics(Ng, Np, Nc):
    ...
    CP_class = Nc / Np
    CR_class = Nc / Ng
    CP = np.sum(CP_class) / n_class
    CR = np.sum(CR_class) / n_class
    CF1 = (2 * CP * CR) / (CP + CR)
    # print(CP_class, CR_class)
    CF1_avg = (2 * CP_class * CR_class) / (CP_class + CR_class)

    return CP, CR, CF1, np.sum(CF1_avg) / n_class
...
def multilabel_sewerml_evaluation(scores, targets, weights, threshold=0.5):
    ...
    CP, CR, CF1, CF1_avg = per_class_metrics(Ng, Np, Nc)
